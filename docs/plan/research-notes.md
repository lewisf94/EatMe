...
- Caution: Exact `tailscale serve` flags drift between releases. P4 says to confirm with `tailscale serve --help` inside the container. The intent: background-serve HTTPS:443 → `http://127.0.0.1:8099`.
...
- Caution: Battery-ADC pin not verified — P6 leaves it as a TODO to read off Seeed's "Buttons, Buzzer, LED, Battery & Low Power" cookbook.
...
### Follow-up: controller, e-paper and solar research (28 July 2026)

- **C3 versus C6:** Espressif specifies 5 uA bare-chip deep sleep for the C3 and 7 uA for the C6, a difference of only about 17.5 mAh/year. Complete-board leakage is more important. Seeed specifies 44 uA for the XIAO C3 but 15 uA for the XIAO C6, so the C6 board is the better low-power choice in that directly comparable product family.
- **Prototype controller:** use the Tenstar Robot ESP32-C6 Super Mini already owned, then measure it. Generic Super Mini revisions do not provide enough reliable whole-board power data to assume they match a branded board. Keep it below approximately 50 uA sleep; replace it with the XIAO C6 if it exceeds 100 uA or has inconsistent Wi-Fi/update behaviour.
- **Controller alternatives:** XIAO ESP32-C6 is the best compact documented option at a published 15 uA board sleep. The original DFRobot FireBeetle ESP32 DFR0478 is advertised at 10 uA and an independent test reported 11 uA, making it a strong battery-life candidate if its 29 x 58 mm size and Micro-USB are acceptable. FireBeetle 2 ESP32-E DFR0654 adds USB-C, more I/O and GDI, but its 10 uA retailer claim conflicts with current DFRobot documentation, which states 2 mA after cutting its low-power jumper; historical documentation states 13 uA. Measure the delivered revision. FireBeetle 2 ESP32-C6 v1.2 is larger and sleeps at a specified 36 uA, but integrates LiPo management and a documented 5 V solar input. Waveshare ESP32-C6-Zero lacks a charger and a published board-level sleep figure.
- **Prototype display:** Waveshare Pico-ePaper-4.2 black/white, 400 x 300. Its separate SPI connector works with ESP32, and its resolution matches the current EatMe endpoint.
- **Preferred final display:** Waveshare 3.97-inch black/white HAT+, 800 x 480. It is 99.5 x 60 mm with its driver, supports partial refresh and is listed by ESPHome. EatMe needs an 800 x 480 render profile first. The 4.26-inch 800 x 480 panel is the larger alternative.
- **Bright-window solar:** Voltaic P123, 66 x 66 mm, 6 V-class and 0.64 W peak. It fits behind the 4.2-inch prototype display. P122 is smaller with less margin; P124 is 1.2 W but extends beyond the display.
- **Indoor solar:** a 50 x 50 mm Epishine indoor cell produces about 175 uW at 200 lux. Eight hours yields only about 1.4 mWh before losses, below the estimated 2–4 mWh/day complete-system budget. Treat the display as solar-assisted until measurements at the real location, including winter, demonstrate otherwise.
- **Power path:** for the Tenstar prototype, use the Adafruit bq25185 USB/DC/solar charger with 5 V boost as the only charger. Leave the controller BAT pads unused and isolate external 5 V before connecting the controller's USB port.
- Full matrices, calculations, primary sources, BOM and safety notes: [e-paper hardware research](../05-hardware-research.md).
...
- 2026-07-20 (P4): No Docker **daemon** in the sandbox (CLI present, socket absent), so the image build, container run, Tailscale and Pi install required hands-on verification. Verified instead the **exact production runtime path on the host**: prod env + `WEB_DIST` (built PWA) + non-default `MIGRATIONS_DIR` + `AUTH_TOKEN` → health open, `/api/*` 401 without/‑wrong bearer and 200 with it, web shell served, seed via the env migrations dir, and the web client sending the token from `localStorage`. Choices: run the server via **`tsx`** in the container (not a tsc/esbuild build — avoids the workspace `.ts` dual-package hazard, fine for a personal add-on); install Tailscale by **copying the binaries from `tailscale/tailscale:stable`** (not `apk`) to dodge Alpine-repo uncertainty; skipped `build.yaml` (the `node:24-alpine`/`tailscale` bases are multi-arch, so the Dockerfile builds for the Pi's arch as-is). `tailscale serve` flag spelling is version-dependent — `run.sh` tries current + fallback forms and logs guidance.
- 2026-07-20 (P3): Self-hosted-wasm path **resolved and proven**: `import "zxing-wasm/reader/zxing_reader.wasm?url"` (Vite emits a hashed same-origin asset, precached by Workbox) + `setZXingModuleOverrides({ locateFile })` from `barcode-detector/pure` (it re-exports that from `zxing-wasm/reader`). Verified with a real decode — Chromium's fake camera fed a generated EAN-13 and the scanner filled the field, with the wasm fetched same-origin and **zero CDN requests**. PWA installability, offline shell, and the camera-denied fallback all pass. The on-device iPhone camera scan still needs hands-on verification over P4 HTTPS. Icons were rendered with Chromium (resvg's native binary isn't linked on this host).
